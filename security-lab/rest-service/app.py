...
ISS = "http://localhost:8081/realms/dsa-lab"
AUD = "rest-client"
...
for i in range(max_retries):
	try:
		KEYS = {k["kid"]: k for k in requests.get(JWKS_URL, timeout=2).json()["keys"]}
		print(f"[REST] JWKS loaded after {i}s")
		break
	except:
		if i < max_retries - 1:
			print(f"[REST] Keycloak not ready (Traefik), retry {i+1}/{max_retries}")
			time.sleep(1)
		else:
			print("[REST] STarting without JWKS - auth will fail until keycloak ready")
...
resource = Resource.create({"service.name": "rest-service"})
...
print(f"[REST] Connecting to gRPC at {GRPC_HOST}:{GRPC_PORT} with mTLS")
with open("/certs/ca.crt", "rb") as f:
	root_certs = f.read()
with open("/certs/rest-service.key", "rb") as f:
	private_key = f.read()
with open("/certs/rest-service.crt", "rb") as f:
	cert_chain = f.read()
...
def verify_token():
	...
	if request.path in ['/healthz', '/health', '/metrics']:
		return
	...
	try:
		claims = jwt.decode(token, public_key, algorithms=[header["alg"]], audience=AUD, issuer=ISS)
	except Exception as e:
		print(f"[REST] JWT Error: {e}")
		abort(401, description="invalid token: {e}")

	request.user = claims.get("preferred_username", claims.get("sub"))
def start_timer():
	# Start timing the request
	g.start_time = time.time()
	g.request_timer = REQUEST_LATENCY.labels(method=request.method, endpoint=request.path).time()
